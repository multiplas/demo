<?php header('Content-type: text/html; charset=utf-8');
		...
		switch ($_GET['sys_action'])
		{
			...
			case 'anadir':		// Añadir producto al carrito.
                                ...
                                if($_GET['addcart'] != '' && $_SESSION['usr'] != null)
				{
                                    ...
                                    {
                                        $nom = str_replace(" ", "_", ereg_replace("[^A-Za-z0-9]", "", $assoc['atributo']));
                                        if ($_POST[$nom] != ''){
                                            $atributos .= $_POST[$nom].' - ';
                                        }
			...
			case 'realizar_pago':
				...
				if ($_SESSION['compra']['pago']['pagov'] == 'ccc')
					PagarConTransferencia($_POST[penvio], $_POST[transp]);
				else if ($_SESSION['compra']['pago']['pagov'] == 'cre')
					PagarContraReembolso($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'tie')
					PagarEnTienda();
                                else if ($_SESSION['compra']['pago']['pagov'] == 'tpv')
					PagarTarjetaSinTPV($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'dom')
					PagarDomiciliacion($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'domm')
					PagarDomiciliacionM($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'fdir')
					PagarDomiciliacionFD($_POST[penvio], $_POST[transp]);
				else if ($_SESSION['compra']['pago']['pagov'] == 'tar')
					PagarConTarjetaTPV($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'paym')
					PagarConPaypalS($_POST[penvio], $_POST[transp]);
                                else if ($_SESSION['compra']['pago']['pagov'] == 'fapl')
					PagarConAplazame($_POST[penvio], $_POST[transp]);
				else
					PagarConPaypal($_POST[penvio], $_POST[transp]);
				break;
			case 'pagook':
    ...
    if ($_SESSION['usr'] != null){
        ...
        $cestanum = CestaSession($_SESSION['cestases']);
    }
...
<html><!-- Estructura HTML5 principal (cambios dinamicos con PHP) -->
    ...
    <script type="text/javascript">
		$.cookie('googtrans', '/en/<?=$_SESSION['lenguaje']?>');
    </script> 
            ...
            if(isset($_GET['lng'])){
				...
				else if (isset($_GET['sys_action']) && $_SESSION['usr'] != null)
				{
					if ($_GET['sys_action'] == 'realizar_pago')
						include_once($draiz.'/secciones/plataforma.html');
					else
