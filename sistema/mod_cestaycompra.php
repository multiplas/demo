...
function Presupuesto($usuario)
	...
	function ProductosComprados($fsec, $uid)
	{
		...
		$sql = "SELECT p.id AS id, c.idcompra AS idcompra, p.nombre AS nombre, p.referencia AS referencia, p.fDirecta AS fDirecta, c.talla AS talla, c.fechas AS fechas, c.extra AS extra, c.precio AS precio, c.cantidad AS cantidad, c.personalizado AS personalizacion, cp.portes AS portes
				FROM bd_compra_productos c, bd_productos p, bd_compra cp
		...
		if ($compraid >= 0)
		{
			if (count($pedido) > 0)
			{
				foreach($pedido AS $producto)
				{
					...
					if (isset($producto['pack']) && $producto['pack'] === true)
					{
                ...
                echo '<html lang="es">
                    ...
                    <body>
                        <form name="frm" action="'.$entornoURL.'" method="POST" target="_self">
                            ...
                            <!--<input type="submit" value="Enviar" >-->Redirigiendo...  Si no redirige haz click <input type="submit" value="aquí"/>
                        </form>
