								...
								<tr>
									...
									<td class="add_cuantity_product">
										...
										<form method="post" name="cesta<?=$micesta['id']?>" action="<?=$draizp?>/acc/actualizar/<?=$micesta['id']?>">
                                                                                    <?php if($Empresa['mmcesta'] == 1){ ?><button class="bag_buttons" style="background-color: <?=$colores2['colorbotonform']?>;" onclick="sumar('cantidad<?=$micesta['id']?>')">+</button><?php } ?>
                                                                                        <input type="text" class="miniF" id="cantidad<?=$micesta['id']?>" name="cantidad" value="<?=$micesta['cantidad']?>"<?=$micesta['id'] > 0 ? '' : ' readonly'?> />
                                                                                    <?php if($Empresa['mmcesta'] == 1){ ?><button class="bag_buttons" style="background-color: <?=$colores2['colorbotonform']?>;" onclick="restar('cantidad<?=$micesta['id']?>')">-</button><?php } ?>
