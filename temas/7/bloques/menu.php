...
<div id="grupo-submenu"  style="width: 100%; -webkit-box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4);-moz-box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4);box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4); min-height: 75px;">
	...
	<div id="submenu" style="top: 50%; -ms-transform: translateY(-50%); -webkit-transform: translateY(-50%); transform: translateY(-50%); font-family: <?=$fuente1?>,sans-serif;">
		<ul>
				...
				foreach ($categorias AS $padre)
				{
                    ...
                    if ($padre['id'] == $bar){
                        if(!empty($_SESSION['lenguaje']))
                            ...
                            $barpath = '<a href="'.$draizp.'/productos/'.$padre['id'].'/'.$nombre.'/"'.(count($padre['categorias']) < 1 ? '' : '').'>'.$padre['nombre'].'</a>';  
                    } 
							...
							foreach ($padre['categorias'] AS $hijo1)
							{
								...
								if ($hijo1['id'] == $bar){
									...
									'<a href="'.$draizp.'/'.$_SESSION['lenguaje'].'/productos/'.$hijo1['id'].'/'.$nombre2.'/"'.(count($hijo1['categorias']) < 1 ? '' : '').'>'.$hijo1['nombre'].'</a>';
									    ...
									    $barpath = '<a href="'.$draizp.'/productos/'.$padre['id'].'/'.$nombre.'/">'.$padre['nombre'].'</a> &gt; '.
									'<a href="'.$draizp.'/productos/'.$hijo1['id'].'/'.$nombre2.'/"'.(count($hijo1['categorias']) < 1 ? '' : '').'>'.$hijo1['nombre'].'</a>';
                                }
					?>
									<li class="sm1">
                                                                            <?php if($hijo1['url'] == ''){ ?>
											...
											if (count($hijo1['categorias']) > 0 && $hijo1['categorias'] != null)
											{
												...
												foreach ($hijo1['categorias'] AS $hijo2)
												{
													...
													if ($hijo2['id'] == $bar){
														...
														'<a href="'.$draizp.'/productos/'.$hijo2['id'].'/'.$nombre3.'/"'.(count($hijo2['categorias']) < 1 ? '' : '').'>'.$hijo2['nombre'].'</a>';
                                                    }
										?>
													<li class="sm2">
                                                                                                            <?php if($hijo2['url'] == ''){ ?>
																		...
																		<?php
																			foreach ($hijo2['categorias'] AS $hijo3)
																			{
																				...
																				if ($hijo3['id'] == $bar){
																					...
																					'<a href="'.$draizp.'/productos/'.$hijo3['id'].'/'.$nombre4.'/"'.(count($hijo3['categorias']) < 1 ? '' : '').'>'.$hijo3['nombre'].'</a>';
                                                                                }
																		?>
			...
			<?php
				}else if ($control == 3){ echo '<li onmouseover="mostrar();" onmouseout="ocultar();" style="font-weight:bold;"><a href="#"><span style="width:15px;height:15px;background-image: url('.$draizp.'/source/th.png);margin-top:15px;"></span>&nbsp;Otros productos</a></li>'; $control++; 
			...
			<?php
				}else if ($control == 5){ echo '<li onmouseover="mostrar();" onmouseout="ocultar();" style="font-weight:bold;"><a href="#"><span style="width:15px;height:15px;background-image: url('.$draizp.'/source/th.png);margin-top:15px;"></span>&nbsp;Otros productos</a></li>'; $control++; } }
