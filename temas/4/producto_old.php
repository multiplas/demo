...
<div id="myModal2" class="modal" onclick="closeModal()" style="opacity: 1; width: auto; height: auto; max-height: 80%; max-width: 85%; position: absolute; left: 50%; top: 50%; position: fixed; transform: translate(-50%, -50%); -webkit-transform: translate(-50%, -50%); background-color: #C0C0C0">
                        ...
                        <div style="display:table; width:100%; border-top: 1px solid rgba(150, 150, 150, 0.3);"><br>
                                                ...
                                                $(document).on("change", "#<?=$atributo['nombre']?>", function(){  
                                                    $.post("/ajax/precio.php", {id: <?=$producto['id']?>, valor: $( "#<?=$atributo['nombre']?>" ).val(), moneda1: '<?=$Empresa['moneda']?>', moneda2: '<?=$_SESSION['divisa']?>' }, function(respuesta){ 
                                                        ...
                                                        if(temp['0'] == 't'){
                                                            ...
                                                            var tot2 = $('#totalspan').attr('data-preciobase');
                                                            var dif = parseFloat(tot) - (parseFloat(tot2) + parseFloat($("#<?=$atributo['nombre']?>").attr('data-precio')));
                                                            var precioAnt = parseFloat(tot) - parseFloat($("#<?=$atributo['nombre']?>").attr('data-precio'));
                                                            ...
                                                            $("#<?=$atributo['nombre']?>").attr('data-precio', precioAnt);
                                                            ...
                                                            var tot = $('#totalspan').attr('data-precio');
                                                            tot = parseFloat(tot) - parseFloat($("#<?=$atributo['nombre']?>").attr('data-precio'));
                                                            if($( "#<?=$atributo['nombre']?>" ).val() != '')
                                                                tot = tot + parseFloat(temp['1']);
                                                            $("#<?=$atributo['nombre']?>").attr('data-precio', temp['1']);
                                                            $('#totalspan').attr('data-precio', tot);
                                                            tot = tot.toFixed(2);
                                                            tot = tot.replace(",","/");
                                                            tot = tot.replace(".",",");
                                                    ...
                                                    $.post("/ajax/foto.php", {id: <?=$producto['id']?>, valor: $( "#<?=$atributo['nombre']?>" ).val()}, function(respuesta){ 
                                                        if(respuesta != ''){
                                                            var dirImg = '/imagenesproductosAtr/'+respuesta;
                                                            $('#frame_cargaD').attr('style', 'display:none');
                                                            $('#imagenAtrb').attr('src', dirImg);
                                                            $('#frame_cargaImg').attr('style', 'display:block');
                                                        }else{
                                                            $('#frame_cargaImg').attr('style', 'display:none');
                                                            $('#frame_cargaD').attr('style', 'display:block');
                                                        }
                                                    });
                                                });
                                ...
                                ?>
                                            ...
                                            <script>
                                                ...
                                                <div style="margin:0 !important;width:100% !important;" class="talla">
                                                    <select id="<?=$atributo['nombre']?>" style="width:100%;" name="<?=$atributo['nombre']?>" <?=@$atributo['obligatorio'] == 'Si' ? '' : ''?> data-precio="<?=@$atributo['obligatorio'] == 'Si' ? $atributo['precio'] : '0'?>"> 
                                                        <?=@$atributo['obligatorio'] == 'Si' ? '' : ($atributo['mensaje'] != '' ? '<option value="" selected>'.$atributo['mensaje'].'</option>':'<option value="" selected>Ninguno/a</option>')?>
                                                        <option value="<?=$atributo['atributo']?>"><?=$atributo['atributo']?></option>
                                                ...
                                                if($('#fsalida').val() != ""){
                                                        $.post("/ajax/precio_esp.php", {id: <?=$producto['id']?>, valor: $( "#<?=$atributo['nombre']?>" ).val(), fentrada: $( "#fentrada" ).val(), fsalida: $( "#fsalida" ).val(), moneda1: '<?=$Empresa['moneda']?>', moneda2: '<?=$_SESSION['divisa']?>' }, function(respuesta){ 
                                                            if(respuesta != "- " && respuesta != "--"){
                                                                ...
                                                                $("#text_info").html("<?=$Empresa['msgdiasmax']?>");
                                                                document.getElementById('myModal').style.display = "block";
                                                                document.getElementById('myModal3').style.display = "block";
                                                            }else{
                                                                tot = respuesta;
		...
		<div class="muestra">
			...
			?>
					<div class="producto">
                        ...
                        <?php //if($_SESSION['usr'] != null || ($_SESSION['usr'] == null && $Empresa['registro'] == 1))
                        if($_SESSION['usr'] != null || ($_SESSION['usr'] == null)){ ?>
                                                <span class="precio" id="precio_r<?=$i?>"><?=$productosRE[$i]['tprecio'] == '' ? number_format(ConvertirMoneda($Empresa['moneda'],$_SESSION['divisa'],$productosRE[$i]['precio']), 2, ',', '.') : $productosRE[$i]['tprecio']?><?=$productosRE[$i]['precio'] != 'Consultar' ? ($productosRE[$i]['tprecio'] == '' ? $_SESSION['moneda'] : '') : ''?></span>
                        <?php } ?>
