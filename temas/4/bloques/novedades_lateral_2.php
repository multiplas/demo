...
<div id="novedades" class="artifmg" style="border: 1px solid rgb(221, 221, 221); padding: 15px;">
	...
	<div class="muestra" style="text-align: center">
        ...
        ?>
                <a href="<?=$draizp?>/<?=$_SESSION['lenguaje']?>producto/<?=$productosMN[$i]['id']?>/<?=str_replace(".", "", str_replace("--", "-", $nombre))?>/">
                    <div id='producto_<?=$i?>' class="<?=$Empresa['ftamano'] == 1 ? 'producto': ''?>" style="border: none !important; -webkit-animation-delay: <?=$i*0.2?>s; -moz-animation-delay: <?=$i*0.2?>s; width: 19% !important; display: inline-table;">
                    ...
                    <?php if($Empresa['actPre'] == 1){ ?><span class="precio"><?=$productosMN[$i]['tprecio'] != '' ? $productosMN[$i]['tprecio'] : ($productosMN[$i]['precio'] != 'Consultar' ? number_format(ConvertirMoneda($Empresa['moneda'],$_SESSION['divisa'],$productosMN[$i]['precio']), 2, ',', '.') : 'Consultar')?><?=$productosMN[$i]['precio'] != 'Consultar' ? $productosMN[$i]['tprecio'] != '' ? '' : $_SESSION['moneda'] : ''?></span><br><?php } ?>
