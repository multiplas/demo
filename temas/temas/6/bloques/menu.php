...
<div id="grupo-submenu"  style="width: 100%; -webkit-box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4);-moz-box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4);box-shadow: 0px 7px 11px 0px rgba(50, 50, 50, 0.4); min-height: 75px;">
	...
	<div id="submenu" style="top: 50%; -ms-transform: translateY(-50%); -webkit-transform: translateY(-50%); transform: translateY(-50%); font-family: <?=$fuente1?>,sans-serif;">
		<ul>
			<li id="casita" class="divLogo2"><a href="<?=$draizp?>/<?php echo !empty($_SESSION['lenguaje'])?$_SESSION['lenguaje'].'/' : ''?>"><i class="fa fa-home fa-lg"></i></a></li>
					...
					<?php
						if (count($padre['categorias']) > 0 && $padre['categorias'] != null)
						{
							...
							foreach ($padre['categorias'] AS $hijo1)
							{
								...
								if ($hijo1['id'] == $bar){
									$barpath = '<a href="'.$draizp.'/';
									if(!empty($_SESSION['lenguaje']))
										$barpath .= $_SESSION['lenguaje'].'/';
									$barpath .= 'productos/'.$padre['id'].'/'.$nombre.'/">'.$padre['nombre'].'</a> &gt; '.
									'<a href="'.$draizp.'/';
									if(!empty($_SESSION['lenguaje']))
										$barpath .= $_SESSION['lenguaje'].'/';
										$barpath .='productos/'.$hijo1['id'].'/'.$nombre2.'/"'.(count($hijo1['categorias']) < 1 ? '' : '').'>'.$hijo1['nombre'].'</a>';
    ...
    <div id="submenu-r">
		...
		<ul id="despl">
			...
			?>
				<li>
					<a href="<?=$draizp?>/<?php echo !empty($_SESSION['lenguaje'])?$_SESSION['lenguaje'].'/' : ''?>productos/<?=$padre['id']?>/<?=$nombre?>/" class="submenu"><?=$padre['nombre']?></a>
					<?php
						if (count($padre['categorias']) > 0 && $padre['categorias'] != null)
						{
					...
					?>         
									<li class="sm1">
										<a href="<?=$draizp?>/<?php echo !empty($_SESSION['lenguaje'])?$_SESSION['lenguaje'].'/' : ''?>productos/<?=$hijo1['id']?>/<?=$nombre2?>/"><?=$hijo1['nombre']?></a>
										<?php
											if (count($hijo1['categorias']) > 0 && $hijo1['categorias'] != null)
											{
												?> <span class="submenu-op">+</span>
