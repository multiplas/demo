				...
				<div class="producto" <?=(($i % 4) == 0) ? 'style="margin-left:0;margin-bottom:1.8em;"' : 'style="margin-bottom:1.8em;"'?>>
					...
					<a href="<?=$draizp?>/<?=$_SESSION['lenguaje']?>/productos/<?=$categorias2[$i]['id']?>/<?=$nombre?>/">
                                            ...
                                            <figcaption style="background-color:<?=$CFefecto?>;">
                                                <h3 class="ih-fade-down ih-delay-sm" style="color:<?=$CLefecto?>"><?=$categorias2[$i]['nombre']?></h3>
                                                <p class="ih-zoom-in ih-delay-sm" style="color:<?=$CLefecto?>"><?=substr($categorias2[$i]['descripcion'], 0, 70)?><?=strlen($categorias2[$i]['descripcion']) > 70 ? '...' : ''?></p>
                                            </figcaption>
                                        </figure>
